...
export default function Business() {
  ...
  const [businessDescribtion, setbusinessDescribtion] = useState<string>("");
  const [layoutType, setLayoutType] = useState<"food" | "market" | "commerce">(
    "food",
  );
  const [orderMode, setOrderMode] = useState<"single_vendor" | "multi_vendor">(
    "single_vendor",
  );
  const [active, setActive] = useState<boolean>();
  const [saveIcon, setSaveIcon] = useState<string>();
  const [saveIconId, setSaveIconId] = useState<number>();
  ...
  async function addBus() {
    ...
    if (!token) return;
    const data = await myClass.addBusiness(token, body);
    setIsLoading(false);
            ...
            <FieldGroup>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field>
                  ...
                  <Select onValueChange={(value) => setLayoutType(value)}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field>
                  ...
                  <Select onValueChange={(value) => setOrderMode(value)}>
                    <SelectTrigger id="form-block">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="single_vendor">
                        Single Vendor
                      </SelectItem>
